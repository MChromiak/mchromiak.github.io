...
Figure 2. A conceptual flow assembled from World Labs' public description. The animation shows evidence entering a shared spatial context, a target camera pose requesting the next element, and rectified flow resolving the new view. It explains the relationship between published components; it is not a diagram of undisclosed internal modules. A [static diagram]({attach}img/atlas-conceptual-flow.svg) is shown when reduced motion is requested. Source: World Labs, [model architecture](https://www.worldlabs.ai/blog/atlas#model-architecture).
{: align=center }
...
Figure 3. The geometric bridge from a depth map to a point cloud. This general camera-geometry diagram explains the reconstruction concept; it is not an Atlas implementation diagram.
{: align=center }
...
Figure 4. An official example associated with Atlas's explicit 3D outputs. The still image conveys appearance, but the original interactive demonstration is needed to inspect view consistency and geometry. Source: World Labs, [explicit 3D outputs](https://www.worldlabs.ai/blog/atlas#explicit-3d-outputs).
{: align=center }
...
Figure 5. Sparse capture for an official Atlas robotics demonstration. Atlas contributes reconstruction and sensor-view generation; the complete workflow also requires robot, simulation, and policy components. Source: World Labs, [robotics simulation](https://www.worldlabs.ai/blog/atlas#robotics-simulation).
{: align=center }
...
Figure 6. Camera-path adherence reported by World Labs. Atlas received explicit camera geometry, whereas the comparison models received text descriptions of the requested motion. The chart evaluates path following, not overall video quality. Data source: World Labs, [Atlas benchmarks](https://www.worldlabs.ai/blog/atlas#benchmarks).
{: align=center }
...
Figures 1, 4, and 5 are reproduced from World Labs' official Atlas materials for commentary and technical explanation. Figures 2 and 3 are original conceptual diagrams; Figure 6 is an original chart of World Labs' reported benchmark data. Copyright in the reproduced material remains with World Labs.
