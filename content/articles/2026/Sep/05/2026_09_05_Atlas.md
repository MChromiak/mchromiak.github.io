...
- **What goes in:** text, images, camera poses, and 3D depth maps. A video is represented as a sequence of images.
...
2. A **simulator** predicts usable state, such as geometry, depth, or physical change.
3. A **planner** selects actions to reach a goal.

Atlas publicly demonstrates strong renderer capabilities and some simulator capabilities. It can synthesize views and output explicit geometry; World Labs also shows it as part of robotics simulation workflows. The launch report does **not** demonstrate Atlas choosing robot actions as a planner. Keeping these roles separate prevents "world model" from becoming a claim that the system already understands every aspect of a physical environment. [World Labs' functional taxonomy](https://www.worldlabs.ai/blog/taxonomy-of-world-models).
...
This resembles a language model predicting the next token, but the unit being generated can be an image or depth map rather than a word fragment. The sequential design also makes familiar inference techniques such as key-value caching relevant, although the launch post does not publish latency or throughput measurements.

### Diffusion: construct a continuous output by denoising

Images contain far more continuous detail than a small vocabulary of text tokens. Atlas uses a latent diffusion approach, specifically **rectified flow**, to generate those outputs by transforming noise toward a sample that fits the spatial context.

Autoregression and diffusion operate at different levels. Autoregression decides which sequence element comes next and conditions it on prior elements. Diffusion constructs the content of that element through iterative denoising. They are complementary, not competing descriptions of the same step.

### Transformer: combine information at scale

The transformer processes relationships across the multimodal sequence. Its matrix-heavy computation maps well to modern accelerators, and it provides an architecture that can scale with data and compute. World Labs reports that larger internal Atlas training runs produced better performance and new capabilities, but it does not publish the model sizes, training curves, or compute used. The scaling statement is therefore a reported development trend, not a public scaling law.
...
World Labs reports generation from one to six reference images, with manually designed paths and output up to one minute at 1440p. "Pixel-perfect camera control" is the company's description of path control; it should not be read as a guarantee that every generated pixel is a correct reconstruction of the source scene.

### Reconstruct a scene from sparse views

For reconstruction, Atlas receives images with their camera poses and predicts a 3D point for each input pixel. It can output new 2D views as well as explicit representations such as point clouds and **3D Gaussian splats**. A Gaussian splat represents a scene as many translucent, oriented 3D primitives that can be rendered quickly from new viewpoints.
...
The result is evidence that Atlas connects space and time in visual generation. It is not yet evidence of accurate long-horizon causal prediction under arbitrary actions.
...
This distinction is important: Atlas is not shown independently controlling the robot in the launch report. Its role is to help build or render the environment in which a policy can be trained and evaluated.
...
World Labs says it reproduced all baseline results under a common protocol. Until code, predictions, and a fuller methodology are released or independently evaluated, the appropriate wording is **"World Labs reports"**, not "Atlas has conclusively established state of the art."
...
These are not reasons to dismiss the work. They define what evidence would turn an impressive launch into a reproducible scientific result.

## Why Atlas is worth following

Atlas's most interesting idea is not that it can generate another video. It is that camera-grounded images, depth, and text can live in one expandable context, and that the model can return either pixels or explicit geometry from that context.
...
The caution is equally important: a model that renders a coherent observation is not automatically a faithful simulator, and a simulator is not automatically a planner. Atlas is a strong demonstration of the first two roles moving closer together. The public evidence does not yet show that the full renderer-simulator-planner loop has been solved.

## Related reading

If you are arriving from language-model training, [QLoRA: How to fine-tune large language models with less memory]({filename}../../../2023/May/26/2023_05_26_QLoRA.md) explains a different part of the modern AI stack. QLoRA changes how a pretrained model is adapted under a memory constraint; Atlas changes the modalities and spatial structure that a foundation model learns to generate. The connection is architectural scale, not a shared training method.
