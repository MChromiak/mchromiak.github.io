...
[![Conceptual Atlas workflow: text, camera-grounded images, and depth maps form a spatial context; autoregressive conditioning requests an output, and rectified flow generates image or depth content that can support 2D or 3D results.]({attach}img/atlas-conceptual-flow.svg)]({attach}img/atlas-conceptual-flow.svg)

Figure 2. A conceptual flow assembled from World Labs' public description. It explains the relationship between the published components; it is not a diagram of undisclosed internal modules. Source: World Labs, [model architecture](https://www.worldlabs.ai/blog/atlas#model-architecture).
{: align=center }
