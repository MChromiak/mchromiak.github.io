...
DINOv2 combines a familiar self-supervised training recipe with carefully curated data and engineering at scale to learn one visual representation that works for both whole images and individual image patches.
...
DINOv2 builds directly on [DINO](../../../../2021/May/03/DINO-Emerging-Properties-in-Self-Supervised-Vision-Transformers), [iBOT](https://arxiv.org/abs/2111.07832), [MAE](../../../../2021/Nov/14/Masked-Autoencoders-Are-Scalable-Vision-Learners), [SEERv2](https://arxiv.org/abs/2202.08360), [MSN](https://arxiv.org/abs/2204.07141), [EsViT](https://arxiv.org/abs/2106.09785), and [Mugs](https://arxiv.org/abs/2203.14415). Its contribution is less about inventing a new block than about making these ideas work together at much larger scale. The result closes much of the benchmark gap between self-supervised features and openly available weakly supervised features such as [CLIP](https://arxiv.org/abs/2103.00020), [OpenCLIP](https://github.com/mlfoundations/open_clip), and [SWAG](https://arxiv.org/abs/2201.08371).


## The problem: one backbone, many visual tasks
...
DINOv2 aims to support all three with one encoder. The released family spans ViT-S/14, ViT-B/14, ViT-L/14, and ViT-g/14; the largest backbone has about 1.1 billion parameters. The central question is whether its frozen features are already organized well enough that a simple task-specific readout can use them.
...
The data is still curated. Seed datasets, filtering rules, deduplication, and similarity search determine which images enter the final corpus. It is therefore useful to separate two ideas: **the training signal is label-free; the data pipeline is deliberately constructed**.
...
Each network summarizes a crop in its class token and passes that token through a DINO projection head. After normalization, $p_s$ and $p_t$ are the student's and teacher's probability distributions over learned prototype coordinates. Their cross-entropy is:
...
If every image mapped to the same feature, the matching losses would become useless. DINOv2 counters this collapse by normalizing the student with softmax and centering the teacher output with three iterations of Sinkhorn-Knopp.
...
For video action recognition, the frozen image encoder processes eight frames independently. The evaluation averages their features for UCF-101 and Kinetics-400, while Something-Something v2 concatenates them to preserve temporal order before training the linear classifier. In other words, the experiment tests whether frame-level visual features support action recognition; the pretraining objective itself remains image-based.
...
The multiscale ViT-g/14 setup reaches 53.0 mIoU on ADE20K, close to MAE with end-to-end UperNet fine-tuning at 53.6. Adding the ViT-Adapter and Mask2Former raises DINOv2 to 60.2 mIoU while keeping 66% of the weights frozen; the cited state of the art reaches 62.9. The progression shows both sides of the result: strong spatial information is present in the frozen features, and a more capable trained predictor extracts more of it.
...
With DPT, DINOv2 ViT-g/14 reports RMSE 0.279 on NYU Depth v2, 2.11 on KITTI, and 0.338 when transferring from NYU Depth v2 to SUN RGB-D. Lower RMSE is better. The corresponding reference results in the paper are 0.330, 2.10, and 0.421: DINOv2 improves the NYU and transfer results and is effectively level with the KITTI reference.
...
Several results set the boundary of the paper's claims:
...
DINOv2's main achievement is the complete recipe rather than a single new architectural block: DINO aligns image-level views, iBOT develops local patch features, KoLeo spreads global representations, LVD-142M supplies curated scale, and the implementation makes a 1.1-billion-parameter teacher trainable. Distillation then transfers that representation into smaller models.

The result is a useful division of labor. The frozen backbone provides transferable image and patch features; a lightweight classifier, retrieval rule, or dense predictor turns those features into a task output. Retrieval can use the features directly, while classification, segmentation, and depth estimation train a readout and may gain more from fine-tuning.
